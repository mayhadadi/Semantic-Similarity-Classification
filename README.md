...
```
s3://your-bucket-name/
├── corpus/           # Google Syntactic N-Grams data
├── data/             # Gold standard data
├── jars/             # MapReduce JAR files
├── logs/             # EMR logs
└── output/           # Job outputs
    ├── step1/
    ├── step1_5/
...
# Upload JARs to S3
aws s3 cp target/Step1.jar s3://your-bucket/jars/
aws s3 cp target/Step1_5.jar s3://your-bucket/jars/
aws s3 cp target/SortFeatures.jar s3://your-bucket/jars/
aws s3 cp target/Step2.jar s3://your-bucket/jars/
aws s3 cp target/Step3.jar s3://your-bucket/jars/
aws s3 cp target/Step4.jar s3://your-bucket/jars/
aws s3 cp target/App.jar s3://your-bucket/jars/
```

## Running the Project

To run the complete pipeline on AWS EMR:
...
This will:
1. Initialize the AWS EMR client
2. Create a cluster with 6 m4.large instances
3. Execute all MapReduce steps in sequence
4. Store results in S3 output directories

To run just specific steps, modify the `steps` array in `App.java` before building and running.
